...
export class TipoFaltaFormComponent implements OnInit, AfterContentChecked {
  ...
  public doSelectTipo = (value: any) => {
    if (value === 'Padrao') {
      ...
      this.tipoFaltaForm.controls['igrejaId'].setValue(this.igrejaId);
    }
  }

  exclusaoTipoFalta(tipoFalta: TipoFaltaDTO) {
      Swal.fire({
        title: 'Exclusão',
    ...
    excluir(tipoFalta: TipoFaltaDTO) {
      this.tipoFaltaService.delete(tipoFalta.id).subscribe(
        () => {
