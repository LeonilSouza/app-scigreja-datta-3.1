import { Injectable } from "@angular/core";
import { HttpClient } from "@angular/common/http";
import { API_CONFIG } from "../config/api-config";
import { DepartamentoDTO } from "../models/departamento.dto";
import { Observable, throwError } from "rxjs";
import { catchError, map } from 'rxjs/operators';

@Injectable()
export class DepartamentoService {

    private apiPath: string = `${API_CONFIG.baseUrl}/departamentos`;

    constructor(public http: HttpClient) {
    }

    findAll(): Observable<DepartamentoDTO> {
        return this.http.get<DepartamentoDTO>(this.apiPath)
        .pipe(
          catchError(this.handleError));
      }

      getByPageDepartamentoFromIgreja(igrejaId, nome, page, linesPerPage) {

        return this.http.get(`${API_CONFIG.baseUrl}/departamentos/?igreja=${igrejaId}&nome=${nome}&page=${page}&linesPerPage=${linesPerPage}`)
          .pipe(
            catchError(this.handleError)
        );
      }

    findById(id: number): Observable<DepartamentoDTO> {

        const url = `${this.apiPath}/${id}`;

        return this.http.get(url).pipe(
          catchError(this.handleError),
          map(this.jsonDataToDepartamento)
        )
      }

     create(departamento : DepartamentoDTO) {
      return this.http.post(this.apiPath,
        departamento,
          {
              observe: 'response',
              responseType: 'text'
          }
        );
     }

      update(departamento: DepartamentoDTO): Observable<DepartamentoDTO> {
        const url = `${this.apiPath}/${departamento.id}`;

        return this.http.put(url, departamento)
          .pipe(
           map(this.jsonDataToDepartamento),
           catchError(this.handleError),
           //map(this.jsonDataToDepartamentoFuncao)
           map(() => departamento)
        )
      }


      delete(id: number): Observable<any> {

        const url = `${this.apiPath}/${id}`;

        return this.http.delete(url).pipe(
          catchError(this.handleError),
          map(() => null)
        )
      }


      private jsonDataToDepartamento(jsonData: any): DepartamentoDTO {
        return (new DepartamentoDTO(), jsonData);
      }

      private handleError(error: any): Observable<any>{
        console.log("ERRO NA REQUISIÇÃO => ", error);
        return throwError(error);
      }

}
